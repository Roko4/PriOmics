#' DEPRECATED: Plot network based on package 'visNetwork'
#'
...
#'
#' @export
plot_net_old <- function(x, plot_style, node_color, node_shape){
  ...
  ### get nodes
  ### qgraph
  qgraph_obj <- qgraph::qgraph(x,
                               layout = 'spring',
                               color = node_color,
                               shape = node_shape,
                               arrows = FALSE,
                               DoNotPlot =T,
                               edge.labels = F,  # plot value of edges
                               borders=F, # black border around nodes
                               palette="colorblind",
                               legend=F,
                               labels=colnames(x),
                               label.cex = 1.5,
                               cut = 0, # needs to be defined, since it changes automatically if # nodes > 20
                               esize=1,
                               edge.width=1,
                               vsize=4,
                               bg=FALSE
  ...
  plot_style0 = plot_style # "circle" | "visNetwork" | igraph
  ...
  # layout_nicely | layout_in_circle | layout_with_sugiyama
  if (plot_style0 == "igraph") {
    net <- visNetwork::visIgraphLayout(net, layout = "layout_nicely", physics = FALSE)
  } else if (plot_style0 == "circle") {
    net <- visNetwork::visIgraphLayout(net, layout = "layout_in_circle", physics = FALSE)
